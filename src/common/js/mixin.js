...
import envconfig from "@/server/envconfig.js";
/* eslint-disable */
/**
 * 开启接触touchmove默认事件
 * @param {Element} el
...
// 裁剪数组
export const tailoringArray = function (array, subGroupLength) {
  let index = 0;
  let newArray = [];
...
// 裁剪数组
export const tailoringString = function (str) {
  let imgUrlArr = [];
  if (str.indexOf(",") == -1) {
    if (str.substr(0, 7).toLowerCase() == "http://") {
      imgUrlArr.push(str);
    } else {
      imgUrlArr.push(envconfig.baseImgUrl + str);
    }
  } else {
    let arr = str.split(",");
    arr.forEach(item => {
      if (item.substr(0, 7).toLowerCase() == "http://") {
        ...
        imgUrlArr.push(envconfig.baseImgUrl + item);
      }
    });
  }
  return imgUrlArr;
